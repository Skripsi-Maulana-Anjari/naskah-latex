...
\section*{2. Latensi Baseline Jaringan}
Tabel berikut menunjukkan hasil pengukuran RTT (\textit{Round Trip Time}) menggunakan protokol ICMP dari Control Plane ke masing-masing nodes. Pengukuran dilakukan sebanyak 5 paket per node untuk memastikan konektivitas dasar yang stabil.
...
\section*{3. Penggunaan Penyimpanan Blockchain (Disk Usage)}
Berdasarkan pengukuran pada direktori \texttt{chaindata} (data leveldb), berikut adalah penggunaan ruang penyimpanan per \textit{node} di akhir pengujian. Terlihat adanya perbedaan ukuran antara \textit{Signer/Validator} (rata-rata 1.6 GB) dan \textit{Non-Signer/Non-Validator} (rata-rata 6 GB), yang mengindikasikan beban akumulasi state yang berbeda pada node RPC publik.
...
\section*{4. Struktur Direktori Node}
