...
1 & Choi \& Hong (2020) & PoA vs PoS pada Ethereum privat & Eksperimen performa & TPS, Latensi & PoA unggul dalam latensi rendah dan \textit{throughput} tinggi & Tidak ukur konsumsi sumber daya & Perbandingan awal PoA–PoS di jaringan privat \\ \hline
...
5 & Kumar \textit{et al.} (2020) & Evaluasi performa blockchain dengan Caliper & Eksperimen & TPS, Latensi & Caliper efektif untuk \textit{benchmarking} & Hanya satu konsensus diuji & Validasi alat ukur performa \\ \hline
...
Transaksi dalam \textit{blockchain} merupakan unit data yang merepresentasikan perpindahan aset digital atau informasi lain yang tercatat dalam jaringan. Secara sederhana, transaksi pada \textit{blockchain} dapat dianalogikan dengan catatan dalam buku besar akuntansi: setiap entri mencatat siapa pengirim, apa yang dikirimkan, dan siapa penerimanya. Pada platform Ethereum, transaksi dapat berupa (1) transfer aset digital antar alamat dompet, (2) pemanggilan fungsi \textit{smart contract} seperti pencetakan sertifikat baru, atau (3) penerbitan \textit{smart contract} baru ke dalam jaringan. Dalam konteks DChain, bentuk transaksi utama meliputi pencetakan sertifikat digital (\texttt{mint}), pembacaan data sertifikat (\texttt{read}), serta pencabutan sertifikat (\texttt{burn}).

Dalam konteks umum, data yang tersimpan pada \textit{blockchain} mencakup \textit{hash} transaksi, alamat dompet pengirim dan penerima, \textit{timestamp}, serta data \textit{state} yang diperbarui oleh eksekusi \textit{smart contract}. Pada implementasi DChain, data yang dicatat secara spesifik meliputi \textit{hash} sertifikat akademik (seperti ijazah digital dan transkrip), metadata dokumen akademik (identitas penerbit, tanggal penerbitan, status validitas), serta informasi \textit{microcredential}. Perlu dicatat bahwa \textit{blockchain} pada umumnya tidak menyimpan dokumen secara langsung, melainkan hanya menyimpan \textit{hash} dan metadata yang merujuk pada dokumen asli yang disimpan di luar rantai (\textit{off-chain storage}) sebagaimana dijelaskan lebih lanjut pada Sub-bab 2.2.5. Agar transaksi dapat ditambahkan ke dalam blok, diperlukan proses verifikasi melalui mekanisme konsensus seperti \textit{Proof of Work} (PoW), \textit{Proof of Stake} (PoS), dan \textit{Proof of Authority} (PoA). Konsensus ini berfungsi memastikan seluruh \textit{node} dalam jaringan memiliki salinan data yang sama dan valid tanpa harus mengandalkan pihak pusat \cite{judmayer2018, guo2022, monrat2019}.

Karakteristik utama \textit{blockchain} meliputi: desentralisasi, di mana data didistribusikan ke banyak \textit{node} tanpa otoritas pusat \cite{dong2023, guo2022, monrat2019}; \textit{immutability}, yakni sifat permanen dari data yang sudah tercatat \cite{dong2023, tripathi2023, krichen2022}; transparansi dan auditabilitas, yang memungkinkan setiap transaksi dapat ditelusuri dan diverifikasi oleh seluruh peserta jaringan \cite{dong2023, guo2022, monrat2019}; serta keamanan, yang dicapai melalui algoritma kriptografi dan struktur hash \cite{dong2023, tripathi2023, kc2018}. Struktur dasar \textit{blockchain} diilustrasikan pada Gambar~\ref{fig:blockchain_structure}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/blockchain-structure.png}
    \caption{Struktur dasar \textit{blockchain} yang menunjukkan rangkaian blok yang terhubung melalui \textit{hash pointer}.}
    \label{fig:blockchain_structure}
\end{figure}

Gambar~\ref{fig:blockchain_structure} mengilustrasikan struktur fundamental \textit{blockchain} yang terdiri dari rangkaian blok yang saling terhubung secara kronologis. Setiap blok memuat \textit{block header} yang berisi \textit{previous hash} (penunjuk ke blok sebelumnya), \textit{timestamp} (waktu pembuatan blok), \textit{nonce} (nilai acak untuk proses validasi), dan \textit{Merkle root} (representasi hash dari seluruh transaksi dalam blok). Panah yang mengarah ke kiri menunjukkan bahwa setiap blok menyimpan \textit{hash} dari blok sebelumnya, sehingga membentuk rantai yang tidak dapat diputus tanpa mengubah seluruh rantai setelahnya. Blok pertama (\textit{Genesis Block}) memiliki \textit{previous hash} bernilai nol karena tidak memiliki pendahulu. Variasi jumlah transaksi pada setiap blok menunjukkan bahwa kapasitas transaksi per blok bersifat dinamis sesuai konfigurasi jaringan. 
...
Ekosistem \textit{DChain} secara keseluruhan yang mencakup berbagai pemangku kepentingan dan layanan yang terintegrasi diilustrasikan pada Gambar~\ref{fig:dchain_ecosystem}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/dchain-ecosystem.png}
    \caption{Ekosistem \textit{DChain} yang menunjukkan interaksi antara pemangku kepentingan, layanan, dan infrastruktur \textit{blockchain}.}
    \label{fig:dchain_ecosystem}
\end{figure}

Gambar~\ref{fig:dchain_ecosystem} mengilustrasikan ekosistem \textit{DChain} yang terdiri dari tiga lapisan utama. Lapisan pertama (\textit{Stakeholders}) mencakup para pemangku kepentingan yang berinteraksi dengan sistem, yaitu mahasiswa sebagai pemilik kredensial, institusi pendidikan tinggi sebagai penerbit sertifikat, dan industri/pemberi kerja sebagai pihak yang memverifikasi keaslian dokumen. Lapisan kedua (\textit{Services \& Applications}) menyediakan berbagai layanan seperti penerbitan ijazah digital, verifikasi sertifikat, dan pengelolaan \textit{microcredential}. Lapisan ketiga (\textit{Blockchain Infrastructure}) merupakan fondasi teknis yang terdiri dari jaringan \textit{node} validator yang tersebar di berbagai universitas, menjalankan mekanisme konsensus dan menyimpan data secara terdesentralisasi. Ketiga lapisan ini bekerja secara terintegrasi untuk mewujudkan sistem manajemen kredensial akademik yang aman, transparan, dan terpercaya.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        signer/.style={circle, draw=blue!80, fill=blue!10, thick, minimum size=1.3cm, inner sep=1pt, font=\scriptsize\bfseries},
        non-signer/.style={circle, draw=gray!80, fill=gray!10, dashed, thick, minimum size=1.3cm, inner sep=1pt, font=\scriptsize},
        connection/.style={->, >=stealth, thin, gray!60}
    ]
        % --- PoA Topology ---
        \node (poa_label) at (0, 4.5) {\textbf{PoA (Clique)}};
        
        % Signers (Core)
        \foreach \a/\name [count=\i] in {90/UGM, 162/ITB, 234/UI, 306/UB, 18/ITS} {
            \node[signer] (S\i) at (\a:1.5) {\name};
        }
        % Connections between Signers (Full Mesh in Clique)
        \foreach \i in {1,...,5} {
            \foreach \j in {\i,...,5} {
                \draw[connection, <->] (S\i) -- (S\j);
            }
        }
        
        % Non-Signers (Edge)
        \foreach \a/\name [count=\i] in {90/UT, 162/UNUD, 234/Gundar, 306/UNIMED, 18/UNDIP} {
            \node[non-signer] (N\i) at (\a:3.5) {\name};
            % Connection from Core to Edge
            \draw[connection] (S\i) -- (N\i);
        }

        % --- Separator ---
        \draw[thick, dashed, gray!30] (5, 4.5) -- (5, -4);

        % --- PoS Topology ---
        \begin{scope}[xshift=10cm]
            \node (pos_label) at (0, 4.5) {\textbf{PoS (Gasper)}};
            
            % All 10 Nodes (Validators & Non-Validators in Mesh)
            \foreach \a/\name [count=\i] in {18/UGM, 54/ITB, 90/UI, 126/UB, 162/ITS, 198/UT, 234/UNUD, 270/Gundar, 306/UNIMED, 342/UNDIP} {
                \node[signer] (V\i) at (\a:3) {\name};
            }

            
            % P2P Connections (Full Mesh)
            \foreach \i in {1,...,10} {
                \foreach \j in {\i,...,10} {
                    \ifnum\i=\j\else
                        \draw[connection, <->] (V\i) -- (V\j);
                    \fi
                }
            }
...
\textit{Proof of Authority} (PoA) merupakan salah satu mekanisme konsensus \textit{blockchain} yang menggunakan validator terbatas dan otoritatif untuk memverifikasi transaksi serta membentuk blok baru. Secara sederhana, PoA dapat dianalogikan seperti sistem notaris, di mana hanya pihak-pihak yang sudah terpercaya dan dikenal identitasnya yang berhak mengesahkan dokumen. Dalam konteks \textit{blockchain}, pihak-pihak ini disebut sebagai validator. Tidak seperti \textit{Proof of Work} (PoW) yang mengandalkan daya komputasi atau \textit{Proof of Stake} (PoS) yang didasarkan pada jumlah aset, PoA menitikberatkan pada identitas dan reputasi validator yang telah ditunjuk. Model ini menjadikan PoA sangat sesuai untuk \textit{blockchain} privat maupun konsorsium, termasuk aplikasi di sektor pendidikan seperti \textit{DChain}.
...
Kecepatan Transaksi & Sangat tinggi (latensi rendah, \textit{throughput} tinggi) & Rendah – konfirmasi transaksi lambat & Sedang–tinggi – lebih cepat dari PoW, lebih lambat dari PoA & \cite{joshi2021, chikezie2021} \\ \hline
...
Tabel~\ref{tab:perbandingan_konsensus} menunjukkan bahwa dibandingkan PoW dan PoS, PoA unggul dalam efisiensi energi dan kecepatan transaksi, namun lemah dalam desentralisasi dan berpotensi menghadapi risiko sentralisasi. Untuk memberikan gambaran visual yang lebih jelas mengenai perbedaan karakteristik ketiga mekanisme konsensus tersebut, Gambar~\ref{fig:consensus_comparison} menyajikan perbandingan dalam bentuk diagram.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/consensus-comparison.png}
    \caption{Perbandingan karakteristik mekanisme konsensus PoW, PoA, dan PoS.}
    \label{fig:consensus_comparison}
\end{figure}

Gambar~\ref{fig:consensus_comparison} mengilustrasikan perbandingan tiga mekanisme konsensus utama dalam \textit{blockchain}. \textit{Proof of Work} (PoW) mengandalkan kompetisi komputasi antar penambang untuk memvalidasi blok, menghasilkan desentralisasi tinggi namun dengan konsumsi energi yang besar. \textit{Proof of Authority} (PoA) menggunakan validator terpercaya yang telah diotorisasi, memberikan efisiensi tinggi dan latensi rendah, tetapi dengan tingkat desentralisasi yang terbatas. \textit{Proof of Stake} (PoS) memilih validator berdasarkan jumlah aset yang di-\textit{stake}, menawarkan keseimbangan antara efisiensi energi dan desentralisasi. Pemahaman terhadap karakteristik masing-masing konsensus ini menjadi dasar penting dalam menentukan mekanisme yang paling sesuai untuk konteks aplikasi tertentu, termasuk jaringan \textit{DChain}.
...
\textit{Proof of Stake} (PoS) merupakan mekanisme konsensus \textit{blockchain} yang memilih validator berdasarkan jumlah aset atau \textit{stake} yang dimiliki dan dikunci dalam jaringan. Secara sederhana, PoS dapat dianalogikan dengan sistem pemungutan suara berbobot, di mana peserta yang memiliki "jaminan" (\textit{stake}) lebih besar memiliki peluang lebih tinggi untuk dipilih sebagai pengesah transaksi. Semakin banyak aset yang dikunci, semakin besar tanggung jawab dan kepercayaan yang diberikan. Mekanisme ini hadir sebagai alternatif dari \textit{Proof of Work} (PoW) yang dikenal boros energi, dengan tujuan menghadirkan model validasi yang lebih efisien, hemat energi, dan berpotensi lebih desentralisasi \cite{zhang2025, li2023, feng2023}.
...
Arsitektur lengkap Hyperledger Caliper yang menggambarkan interaksi antara komponen-komponen tersebut diilustrasikan pada Gambar~\ref{fig:caliper_architecture}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/caliper-architecture.png}
    \caption{Arsitektur \textit{high-level Hyperledger Caliper} yang menunjukkan alur kerja \textit{benchmarking} dari konfigurasi hingga laporan \cite{hyperledger_caliper}.}
    \label{fig:caliper_architecture}
\end{figure}

Gambar~\ref{fig:caliper_architecture} mengilustrasikan arsitektur \textit{high-level Hyperledger Caliper} berdasarkan dokumentasi resmi. Pada bagian atas, terdapat tiga komponen input utama: \textit{Workload Module} yang mendefinisikan logika transaksi yang akan dieksekusi, \textit{Benchmark Configuration} yang mengatur parameter pengujian seperti jumlah transaksi dan \textit{send rate}, serta \textit{Network Configuration} yang berisi informasi koneksi ke jaringan \textit{blockchain} target. Komponen \textit{Caliper} di tengah bertindak sebagai \textit{engine} utama yang memproses ketiga input tersebut dan mengirimkan \textit{workload} ke \textit{System Under Test} (SUT), yaitu jaringan \textit{blockchain} yang diuji. Selama eksekusi, Caliper juga melakukan \textit{monitoring} terhadap SUT untuk mengumpulkan metrik performa. Di sisi kiri, \textit{Benchmark Artifacts} menyimpan data pendukung seperti \textit{smart contract} dan \textit{state} awal jaringan. Setelah pengujian selesai, Caliper menghasilkan \textit{Benchmark Report} (bagian bawah) yang berisi ringkasan metrik performa seperti \textit{throughput}, latensi, dan tingkat keberhasilan transaksi.
...
Dalam pengelolaan sistem terdistribusi kompleks seperti \textit{blockchain}, pendekatan observabilitas melampaui \textit{monitoring} tradisional. Jika \textit{monitoring} menjawab pertanyaan "apakah sistem sehat?", observabilitas menjawab "mengapa sistem tidak sehat?" dengan menganalisis \textit{output} eksternal sistem \cite{picco2021}. Tiga pilar utama observabilitas adalah \textit{Metrics}, \textit{Logs}, dan \textit{Traces}. Penelitian ini secara spesifik berfokus pada pilar \textit{Metrics} untuk analisis performa kuantitatif. Ketiga pilar observabilitas tersebut diilustrasikan pada Gambar~\ref{fig:observability_pillars}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/observability-pillars.png}
    \caption{Tiga pilar observabilitas sistem terdistribusi: \textit{Metrics}, \textit{Logs}, dan \textit{Traces}.}
    \label{fig:observability_pillars}
\end{figure}

Gambar~\ref{fig:observability_pillars} mengilustrasikan tiga pilar utama observabilitas dalam sistem terdistribusi. \textit{Metrics} menyediakan data kuantitatif agregat seperti \textit{throughput}, latensi, penggunaan CPU, dan memori yang memungkinkan analisis tren performa dari waktu ke waktu. \textit{Logs} merekam kejadian diskret dalam sistem secara kronologis, berguna untuk \textit{debugging} dan audit. \textit{Traces} melacak alur permintaan (\textit{request}) yang melewati berbagai komponen sistem, membantu mengidentifikasi \textit{bottleneck} pada arsitektur \textit{microservices}. Dalam konteks penelitian ini, fokus utama diberikan pada pilar \textit{Metrics} karena paling relevan untuk evaluasi performa kuantitatif mekanisme konsensus \textit{blockchain}. Kombinasi ketiga pilar ini memungkinkan pemahaman menyeluruh terhadap perilaku dan kesehatan sistem terdistribusi.
...
Penelitian ini mengukur lima aspek kinerja utama jaringan \textit{blockchain}, yaitu: (1) \textit{throughput} (jumlah transaksi berhasil per detik), yang mengukur kapasitas pemrosesan jaringan; (2) latensi (waktu tunda dari pengiriman hingga konfirmasi transaksi), yang mengukur responsivitas sistem; (3) penggunaan sumber daya (CPU, RAM, \textit{Disk I/O}, dan \textit{bandwidth}), yang mengukur efisiensi infrastruktur; (4) skalabilitas, yang menilai kemampuan sistem menangani peningkatan jumlah \textit{node} dan klien; serta (5) stabilitas, yang mengevaluasi konsistensi performa pada beban berkelanjutan melalui \textit{soak test}. Kelima metrik ini dipilih karena bersifat umum, netral terhadap jenis konsensus, dan dapat direplikasi sebagaimana dibahas lebih lanjut pada Bab III. Evaluasi kinerja \textit{blockchain} tersebut memerlukan pemahaman mendalam tentang berbagai metrik dan fenomena unik berikut:
...
Jumlah Node Validator & Meningkatkan desentralisasi dan keamanan; menambah redundansi jaringan & Menurunkan \textit{throughput} karena \textit{overhead} komunikasi meningkat & \cite{kaur2021, hao2018} \\ \hline
...
Ukuran Blok (Block Size) & Blok lebih besar menampung lebih banyak transaksi, \textit{throughput} meningkat & Propagasi lebih lambat, risiko \textit{fork} lebih tinggi & \cite{shahsavari2022, gobel2017} \\ \hline
Beban Transaksi (Load) & \textit{Throughput} meningkat seiring beban, hingga titik jenuh & Setelah batas, latensi meningkat drastis, sistem jadi tidak stabil & \cite{thakkar2018, ucbas2023} \\ \hline
Jumlah Worker (Caliper) & Paralelisme transaksi meningkat, simulasi beban lebih realistis & Terlalu banyak \textit{worker} dapat memicu \textit{overload} dan \textit{bottleneck} & \cite{thakkar2018, ucbas2023} \\ \hline
Jenis Transaksi & Operasi baca lebih ringan; tulis dan komputasi intensif menambah konsumsi CPU/I/O & Beban tinggi pada operasi tulis/komputasi menurunkan \textit{throughput} & \cite{dinh2017, gaba2022} \\ \hline
Infrastruktur (VPS/OS) & CPU, RAM, \textit{bandwidth}, dan \textit{containerization} memengaruhi efisiensi dan skalabilitas & Infrastruktur lemah menyebabkan \textit{bottleneck}; konfigurasi salah menurunkan validitas eksperimen & \cite{zheng2019, wu2022} \\ \hline
\end{tabular}}
\end{table}
...
\textbf{Hyperledger Caliper} & TPS, \textit{latency}, \textit{resource usage}, \textit{scalability}, \textit{stability} & Ethereum, Fabric, Besu, Quorum & \textit{Open-source}, modular, integrasi \textit{Prometheus/Grafana}, aktif dipelihara & Instalasi kompleks, kurva belajar tinggi \\ \hline
\textbf{Blockbench} & \textit{Throughput}, \textit{latency}, \textit{scalability}, \textit{fault tolerance} & Ethereum (geth), Parity, Fabric & \textit{Framework} komprehensif, cocok untuk \textit{private blockchain} & Tidak aktif dikembangkan, \textit{workload} terbatas \\ \hline
\textbf{PERSECUS} & \textit{TPS}, \textit{latency}, \textit{CPU/RAM}, \textit{gas}, \textit{energy} & Ethereum, Fabric & Fokus pada \textit{reproducibility}, \textit{stress testing} modular & Dokumentasi publik minim \\ \hline
\textbf{Diablo} & \textit{Latency}, \textit{gas cost}, \textit{error resilience} & Ethereum, Fabric, Tendermint & Kuat dalam skenario \textit{fault injection}, \textit{workload scripting} fleksibel & Masih tahap pengembangan awal \\ \hline
\textbf{Gromit} & \textit{Throughput}, \textit{storage overhead}, \textit{metadata cost} & Ethereum, Fabric & Analisis makro lintas platform, fokus \textit{metadata} & Komunitas kecil, kurang populer \\ \hline
\textbf{BlockCompass} & \textit{Throughput}, \textit{resource efficiency} & \textit{Enterprise Blockchain} & Integrasi \textit{enterprise}, dukungan \textit{workload} realistis & Tidak \textit{open-source}, akses terbatas \\ \hline
\textbf{Grafana + Prometheus} & \textit{CPU}, \textit{RAM}, \textit{bandwidth} (\textit{monitoring} eksternal) & Agnostik (umum) & Visualisasi kuat, fleksibel lintas platform & Tidak menghasilkan \textit{workload}, hanya \textit{monitoring} \\ \hline
\end{tabular}}
\end{table}
...
\textbf{Resource Usage} & Konsumsi CPU, memori, dan \textit{bandwidth} & Menggambarkan efisiensi pemanfaatan infrastruktur; relevan untuk biaya operasional & \cite{dinh2017, kuzlu2019} \\ \hline
...
\textbf{Jumlah Worker (Caliper)} & Klien/\textit{worker} yang mengirim transaksi dalam eksperimen & Menentukan variasi beban transaksi; mereplikasi kondisi multi-klien dalam sistem nyata & \cite{thakkar2018, kuzlu2019} \\ \hline
